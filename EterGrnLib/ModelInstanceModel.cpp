...
granny_world_pose* CGrannyModelInstance::__GetWorldPosePtr() const
{
    ...
    if (m_ppkSkeletonInst && *m_ppkSkeletonInst) {
        if (!(*m_ppkSkeletonInst)->m_pgrnWorldPoseReal) {
            TraceError("Skeleton instance world pose is NULL");
        }
        return (*m_ppkSkeletonInst)->m_pgrnWorldPoseReal;
    }

    TraceError("Both m_pgrnWorldPoseReal and m_ppkSkeletonInst are NULL");
    return NULL;
}
...
{
	// Check if the iBone value is valid before proceeding
	if (iBone < 0) {
		TraceError("Invalid bone index: %d", iBone);
		return NULL;
	}

	// Ensure that the world pose pointer is not NULL before passing it to the external function
	granny_world_pose* worldPosePtr = __GetWorldPosePtr();
	if (!worldPosePtr) {
		TraceError("World pose pointer is NULL for bone index: %d", iBone);
		return NULL;
	}

	const float* bones = GrannyGetWorldPose4x4(worldPosePtr, iBone);
	if (!bones) {
		granny_model* pModel = m_pModel->GetGrannyModelPointer();
		if (!pModel) {
			TraceError("Failed to get Granny model pointer for bone index: %d", iBone);
			return NULL;
		}
