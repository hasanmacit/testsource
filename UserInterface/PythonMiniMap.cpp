...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		else if ((pkInstEach->IsPC() && !pkInstEach->IsInvisibility()))
		{
		...
		else if (pkInstEach->IsNPC() && pkInstEach->IsShop() == false)
		{
#ifdef ENABLE_DUNGEON_BOSS_ICON_IN_MAP
...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
	...
	if (m_fScale >= 2.0f)
	{
		...
		{
			TMarkPosition& rPosition = *aIterator;
			STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_NUM));
			m_WhiteMark.SetPosition(rPosition.m_fX, rPosition.m_fY);
			m_WhiteMark.Render();
			++aIterator;
		}
	...
	STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_PC));
	...
	STATEMANAGER.SaveTextureStageState(0, D3DTSS_MINFILTER, D3DTEXF_ANISOTROPIC);
	STATEMANAGER.SaveTextureStageState(0, D3DTSS_MAGFILTER, D3DTEXF_ANISOTROPIC);
#endif
...
bool CPythonMiniMap::Create()
{
	...
	//m_WhiteMark.SetScale(0.25, 0.25);
	...
	* pReturnPosY = fRealY;
	...
	while (m_AtlasMarkInfoVectorIterator != m_AtlasBossInfoVector.end())
	{
			...
			rAtlasMarkInfo.m_fY - fCheckWidth / 2 < fRealY && rAtlasMarkInfo.m_fY + fCheckHeight > fRealY)
		{
			...
			*pReturnPosX = rAtlasMarkInfo.m_fX;
			*pReturnPosY = rAtlasMarkInfo.m_fY;
			*pdwTextColor = CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_PC);
...
void CPythonMiniMap::AddPartyPositionInfo(const TPartyPosition& Info)
{
	...
	if (it == mAtlasPartyPlayerMark.end())
	{
		...
		const std::string strImageRoot = "D:/ymir work/ui/";
		const std::string strPartyPlayerMark = strImageRoot + "minimap/playerpartymark.dds";
