...
void CCullingManager::VisibilityCallback(const Frustum& f, SpherePack* sphere, ViewState state)
{
	...
	if (pInstance->GetType() == ACTOR_OBJECT)
	{
		state = f.ViewVolumeTestEffects(sphere->GetCenter(), sphere->GetRadius());
	}

	if (state == VS_OUTSIDE)
	{
		const float EPSILON = 1e-5f;  // You can adjust this value as needed

		if (fabs(pInstance->GetPosition().x) > EPSILON)
		{
			pInstance->Hide();
		}
	}
