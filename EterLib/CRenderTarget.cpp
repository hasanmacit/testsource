...
void CRenderTarget::SetRenderingBox(RECT* rect) const
{
	m_renderTargetTexture->SetRenderingBox(rect);
}
...
void CRenderTarget::SelectModel(const int index, bool isFromScale)
{
    ...
    if (index > (NRaceData::JOB_MAX_NUM * 2) && !m_pModel->IsShop())
    {
...
void CRenderTarget::RenderModel()
{
	...
	const auto near_y = python_graphic.GetNearY();
	const auto far_y = python_graphic.GetFarY();
...
void CRenderTarget::SetMotionStatus(bool bFlag)
{
	m_bMotion = bFlag;
	if (m_pModel)
		m_pModel->Refresh(CRaceMotionData::NAME_WAIT, false);
}
