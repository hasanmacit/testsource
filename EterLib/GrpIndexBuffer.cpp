...
bool CGraphicIndexBuffer::Create(int faceCount, TFace* faces)
{
	...
	WORD* dstIndices;
	if (FAILED(m_lpd3dIdxBuf->Lock(0, 0, DXLOCKTYPE&dstIndices, 0)))
		return false;
...
#else
	if (FAILED(ms_lpd3dDevice->CreateIndexBuffer(m_dwBufferSize, D3DUSAGE_WRITEONLY, m_d3dFmt, D3DPOOL_MANAGED, &m_lpd3dIdxBuf)))
#endif
		return false;
