...
int						CGraphicBase::ms_fShadowIntensity;
//float					CGraphicBase::ms_fLeaves;
...
bool CGraphicBase::SetPDTStream(SPDTVertexRaw* pSrcVertices, UINT uVtxCount)
{
	...
	if (FAILED(plpd3dFillRectVB->Lock(0, sizeof(TPDTVertex) * uVtxCount, DXLOCKTYPE&pDstVertices, D3DLOCK_DISCARD)))
	{
...
int CGraphicBase::GetShadowIntensity()
{
	static int previousShadowIntensity = ms_fShadowIntensity;
	static bool wasRainIntensitySet = false;

	if (Frustum::Instance().selectRainFrustun == 16 ||
		Frustum::Instance().selectRainFrustun == 25 ||
		Frustum::Instance().selectRainFrustun == 8)
	{
		if (!wasRainIntensitySet) {
			previousShadowIntensity = ms_fShadowIntensity;
			wasRainIntensitySet = true;
		}
		return 93;
	}
	else
	{
		if (wasRainIntensitySet) {
			ms_fShadowIntensity = previousShadowIntensity;
			wasRainIntensitySet = false;
		}
		return ms_fShadowIntensity;
	}
}
...
float CGraphicBase::GetNearY() const
{
	return ms_fNearY;
}

float CGraphicBase::GetFarY() const
{
	return ms_fFarY;
}
