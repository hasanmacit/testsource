...
namespace UI
{
	...
	void CWindow::IsTransparentOnPixel(long* x, long* y, bool* ret)
	{
		if (IsShow() && IsIn(*x, *y)) // check if the window is active and the cursor is in the window
		{
			...
			{
				D3DXCOLOR pixel = ((CExpandedImageBox*)this)->GetPixelColor2(*x - m_rect.left, *y - m_rect.top); // get current pixel color
...
/**/#ifdef ENABLE_WIKI
	...
	void CTextLine::SetText(const char* c_szText)
	{
		...
		std::string strText = c_szText;
		OnSetText(string_replace_all(strText, "[ENTER]", "\\n").c_str());
#else
		OnSetText(c_szText);
#endif
	}
	...
	void CTextLine::OnUpdateRenderBox()
	{
	...
	void CUiRenderTarget::OnUpdateRenderBox()
	{
		if (m_dwIndex == -1)
			return;
