...
bool CMapOutdoor::Update(float fX, float fY, float fZ)
{
	...
	Frustum::Instance().characterPosition = m_v3Player;
	//TraceError("%f, %f, %f", m_v3Player.x, m_v3Player.y, m_v3Player.z);
	//TraceError("Position: %f, %f, %f,", Frustum::Instance().characterPosition.x, Frustum::Instance().characterPosition.y, Frustum::Instance().characterPosition.z);
	...
	if (bNeedInit ||
		...
		(m_CurCoordinate.m_sTerrainCoordY / LOAD_SIZE_WIDTH) != (sCoordY / LOAD_SIZE_WIDTH) || refreshObjects == true)
	{
		if (bNeedInit)
		{
	...
	if ( bNeedInit ||
		...
		(m_CurCoordinate.m_sTerrainCoordY/LOAD_SIZE_WIDTH) != (sCoordY/LOAD_SIZE_WIDTH) || refreshObjects == true)
	{
		if (bNeedInit || refreshObjects == true)
		{
			m_PrevCoordinate.m_sTerrainCoordX = sCoordX;
			m_PrevCoordinate.m_sTerrainCoordY = sCoordY;
		}
