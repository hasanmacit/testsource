...
void CMapOutdoor::__SoftwareTransformPatch_RenderPatchNone(SoftwareTransformPatch_SRenderState& rkTPRS, long patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
	...
	if (FAILED(pkVB->Lock(0, dwVBSize, DXLOCKTYPE & akDstVertex, D3DLOCK_DISCARD)
	)) return;
...
void CMapOutdoor::__SoftwareTransformPatch_BuildPipeline(SoftwareTransformPatch_SRenderState& rkTPRS)
{
	...
	D3DXVECTOR3 kFogNearVector2;
	D3DXVECTOR3(0.0f, 0.0f, -rkTPRS.m_fFogNearDistance) = kFogNearVector2;
	D3DXVec3TransformCoord(&kFogNearVector, &kFogNearVector2, &rkTPRS.m_m4Proj);

	D3DXVECTOR3 kFogFarVector;
	D3DXVec3TransformCoord(&kFogFarVector, &kFogNearVector2, &rkTPRS.m_m4Proj);
...
bool CMapOutdoor::__SoftwareTransformPatch_SetSplatStream(SoftwareTransformPatch_STLVertex* akSrcVertex)
{
	...
	if (FAILED(pkVB->Lock(0, dwVBSize, DXLOCKTYPE & akDstVertex, 0)//D3DLOCK_DISCARD)
	)) return false;
...
#ifdef DIRECTX9
			if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(sizeof(SoftwareTransformPatch_SSplatVertex) * CTerrainPatch::TERRAIN_VERTEX_COUNT, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR | D3DFVF_TEX2, D3DPOOL_SYSTEMMEM, &m_kSTPD.m_pkVBSplat[uIndex], NULL)))
#else
			if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(sizeof(SoftwareTransformPatch_SSplatVertex) * CTerrainPatch::TERRAIN_VERTEX_COUNT, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR | D3DFVF_TEX2, D3DPOOL_SYSTEMMEM, &m_kSTPD.m_pkVBSplat[uIndex])))
#endif
				return false;
		}
	}
...
#ifdef DIRECTX9
			if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(sizeof(SoftwareTransformPatch_STVertex) * CTerrainPatch::TERRAIN_VERTEX_COUNT, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW, D3DPOOL_SYSTEMMEM, &m_kSTPD.m_pkVBNone[uIndex], NULL)))
#else
			if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(sizeof(SoftwareTransformPatch_STVertex) * CTerrainPatch::TERRAIN_VERTEX_COUNT, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW, D3DPOOL_SYSTEMMEM, &m_kSTPD.m_pkVBNone[uIndex])))
#endif 
				return false;
		}
	}
