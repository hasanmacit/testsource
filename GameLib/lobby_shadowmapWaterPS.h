const char* lobby_shadowmapWaterPS = R"(
struct PS_INPUT
{
...
static const float3 pointLightPositionBlue[4] =
{
    float3(6000.000000, -13100.000000, 16725.500000 + 1000.0f),
    float3(5400.000000, -10200.000000, 16725.500000 + 1000.0f),
    float3(10300.000000, -8800.000000, 16725.500000 + 1000.0f),
    float3(13300.000000, -8800.000000, 16725.500000 + 1000.0f)
};
...
// Compute specular reflection based on alpha intensity
        ...
        float SpecularIntensity = Color.a * pow(saturate(dot(V, R)), Shininess * 2);
...
// Blend the two specular colors based on the Fresnel term
        ...
        if (enableEffects < 0.5)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                {
				// Quadratic falloff for intensity (intense near the center, zero on the margins of 500.0)
                    ...
                    float blueSpecMultiplier2 = 10.6f; // Multiplier to scale the intensity
                    float blueSpecIntensity2 = blueSpecMultiplier2 * falloff2 * pow(saturate(dot(specularReflection2, toSpecular2)), 1.5f); // Adjusted power for tighter highlight
                    float3 blueSpecColor2 = float3(1.0f, 0.5f, 0.2f);
                    LightResult += blueSpecIntensity2 * blueSpecColor2;
                }
            }
        }
        
        if (finalShadow < 0.15f)
        {
            LightResult += SpecularCombined;
            LightResult.rgb *= ChromaticAberrationColor.rgb;
        }
    }
#endif
