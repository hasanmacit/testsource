const char* blue_shadowmapWaterPS = R"(
struct PS_INPUT
{
...
static const float3 pointLightPositionBlue[3] = 
{
    float3(44186.144531, -48116.437500, 16899.125000 + 100.0f),
    float3(42908.312500, -48126.847656, 16912.650391 + 100.0f),
    float3(41052.386719, -47537.515625, 17012.162109 + 100.0f)
};
        ...
        if (enableEffects < 0.5)
        {
            for (int i = 0; i < 3; ++i)
            {
				...
				// Quadratic falloff for intensity (intense near the center, zero on the margins of 500.0)
                    ...
                    float blueSpecMultiplier2 = 2.6f / reduceLightIntensity; // Multiplier to scale the intensity
                    float blueSpecIntensity2 = blueSpecMultiplier2 * falloff2 * pow(saturate(dot(specularReflection2, toSpecular2)), 0.0f); // Adjusted power for tighter highlight
                    float3 blueSpecColor2 = float3(1.0f, 0.6f, 0.2f);
