...
bool IS_NO_PUSHING_MOB(unsigned int vnum)
{
	switch (vnum)
	{
		case 6192:
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
			...
			if (!Frustum::Instance().isIndoor)
			{
				if (Frustum::Instance().bloodLevel == 0)
				{
					if (!rVictim.IsStone())
					{
						...
						if (m_isMain)
						{
							if (rVictim.IsIceBlood())
							{
								if (frandom(0.0f, 1.0f) < 0.5f)  // 50% chance for each
								{
									rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/bloodIce/blood.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
								}
								else
								{
									rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/bloodIce/bloodjtx.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
								}
							}
							else if (rVictim.IsTreeBlood())
							{
								if (frandom(0.0f, 1.0f) < 0.5f)  // 50% chance for each
								{
									rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/bloodTree/blood.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
								}
								else
								{
									rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/bloodTree/bloodjtx.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
								}
							}
							else
							{
								if (frandom(0.0f, 1.0f) < 0.5f)  // 50% chance for each
								{
									rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/blood.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
								}
								else
								{
									rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/bloodjtx.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
								}
							}
						}

						if (rVictim.IsMainInstance())
						{
							if (frandom(0.0f, 1.0f) < 0.5f)  // 50% chance for each
							{
								rkEftMgr.CreateEffect("d:/ymir work/zone/plechi_env/halloween/effects/blood.mse", D3DXVECTOR3(vec3Effect.x + randomFloat, vec3Effect.y + randomFloat2, fGrountHeight), D3DXVECTOR3(0.0f, 0.0f, randomFlotRotation));
	...
	else
	{
		///////////
		// Motion
		if (NRaceData::HIT_TYPE_GOOD == c_rAttackData.iHittingType || rVictim.IsResistFallen() || IS_HUGE_RACE(rVictim.GetRace()) || IS_NO_PUSHING_MOB(GetRace()))
		{
			__HitGood(rVictim);
		}
...
float CActorInstance::__GetInvisibleTimeAdjust(const UINT uiSkill, const NRaceData::TAttackData& c_rAttackData) {
	...
	static const int ninjam = 5, ninjaf = 1;

	if ((GetRace() != shamanw && GetRace() != shamanm && GetRace() != ninjam && GetRace() != ninjaf) ||
		uiSkill != 0 ||
		m_fAtkSpd < 1.3)
		return 0.0f;
