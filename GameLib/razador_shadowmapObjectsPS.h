const char* razador_shadowmapObjectsPS = R"(
struct PS_INPUT
{
...
float4 main(PS_INPUT input) : SV_TARGET
{
	...
	if (SpecularLight > 0.5f) 
	{
        ...
        // Quadratic falloff for intensity (intense near the center, zero on the margins of 2000.0)
                    ...
                    float blueSpecMultiplier2 = 5.0f; // Multiplier to scale the intensity
        ...
        // If there's no alpha channel, reduce the intensity
                    ...
                    float highShininess = 30.0f; // Very high shininess for tighter highlight
                    float blueSpecIntensityHigh = blueSpecMultiplier2 * 1.5f * falloff2 * pow(saturate(dot(specularReflection2, toSpecular2)), highShininess) * flicker2;
