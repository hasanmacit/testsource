...
CMemoryPool<CWeaponTrace> CWeaponTrace::ms_kPool;
  ...
  */
	...
	for(int loop = 0; loop<=1; ++loop)
	{
		...
		for(float t = 0; t<=length; t+=dt)
		{
			...
			v.diffuse = D3DXCOLOR(177.0f / 255.0f, 242.0f / 255.0f, 122.0f / 255.0f, (loop) ? (min(max((1.0f - ttt) * (1.0f - ttt) / 2.5 - 0.1f, 0.0f), 1.0f)) / 2.0f : 0.0f);
