...
/*#ifdef ENABLE_PETS_WITHOUT_COLLISIONS
	if (rVictim.IsPet() || rVictim.IsNewPet())
		return FALSE;
#endif*/
	...
	if (rVictim.IsHorse() || rVictim.GetRace() == 65000)
		return FALSE;
#endif

#ifdef JTX_EXTENDED_COLLISIONS

	if (CPythonBackground::Instance().Collision == 0)
	{

	}
	if (CPythonBackground::Instance().Collision == 1)
	{
		if (!rVictim.isAttacking() && rVictim.IsEnemy() && !rVictim.IsMoving() && !isAttacking())
			return false;
	}
	if (CPythonBackground::Instance().Collision == 2)
	{
		if (rVictim.IsEnemy())
			return false;
	}
