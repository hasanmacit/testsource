const char* blue_shadowmapObjectsPS = R"(
struct PS_INPUT
{
	...
	if (SpecularLight > 0.5f) 
	{
		...
		else
		{
			...
			if (dotVR > 0)
			{
				...
				SpecularIntensity = alphaFactor * Color.a * pow(saturate(dotVR), shinessAlpha * 6) / 4;
			}
			else
			{
				...
				SpecularIntensity = alphaFactor * Color.a * pow(saturate(abs(dotVR)), shinessAlpha * 8) / 25;
			}
        ...
        // Quadratic falloff for intensity (intense near the center, zero on the margins of 2000.0)
                    ...
                    float blueSpecIntensity2 = blueSpecMultiplier2 * falloff2 * pow(saturate(dot(specularReflection2, toSpecular2)), 1.4f) * flicker * 3; // Adjusted power for tighter highlight
