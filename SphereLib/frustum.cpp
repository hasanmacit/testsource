...
{
	//Cu cat se va mari view-ul. Posibil 64 sa fie prea mult, eu tin 8.0f. (poti face o setare in client, cu 2x 8x .. 64x)

	float newFrustrumExtension = 1.0f;

	if (!Frustum::Instance().isIndoor)
		newFrustrumExtension = (treeShadow >= 2 && shadowType != 3) ? increaseShadowRangeFrustum : (shadowType == 3) ? 1.0f : 3.0f;
	...
	if (m_bUsingSphere)
	{
		...
		if ((c_fRadius * newFrustrumExtension + m_fRadius) * (c_fRadius * newFrustrumExtension + m_fRadius) < D3DXVec3LengthSq(&v))
		{
			return VS_OUTSIDE;
		}
	}
	...
	for (i = 0; i < count; i++)
	{
		...
		if (distance[i] <= -c_fRadius * newFrustrumExtension)
			return VS_OUTSIDE;
	}
...
ViewState Frustum::ViewVolumeTestEffects(const Vector3d& c_v3Center, const float c_fRadius) const
{
	if (m_bUsingSphere)
	{
		D3DXVECTOR3 v(
