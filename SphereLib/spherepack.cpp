...
void SpherePack::VisibilityTest(const Frustum& f, SpherePackCallback* callback, ViewState state)
{
	if (state == VS_PARTIAL)
	{
		...
		{
			float shadowRange = Frustum::Instance().increaseShadowRangeFrustum;
			float radiusDivisor = 1.0f;

			switch (static_cast<int>(shadowRange)) {
			case 1:
				radiusDivisor = 1.0f;
				break;
			case 2:
			...
			if(Frustum::Instance().shadowType == 3)
				...
				state = f.ViewVolumeTest(mCenter, GetRadius() / radiusDivisor);
