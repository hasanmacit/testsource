...
float CSoundManager::GetEffectsScale()
{
	return m_fEffectsScale;
}
...
void CSoundManager::SetEffectsScale(float fScale)
{
	m_fEffectsScale = fScale;
}
...
void CSoundManager::SetEffectsVolume(float fVolume)
{
	if (m_isSoundDisable)
	{
		m_fEffectsSoundVolume = fVolume;
		return;
	}
...
void CSoundManager::SetEffectsVolumeGrade(int iGrade)
{
	float fVolume = __ConvertGradeVolumeToApplyVolume(iGrade);
...
void CSoundManager::SetEffectsVolumeRatio(float fRatio)
{
	float fVolume = __ConvertRatioVolumeToApplyVolume(fRatio);
	SetEffectsVolume(fVolume);
}
...
{
	if (CSoundManager::Instance().m_fEffectsVolume > 0.9f)
	{
		CSoundManager::Instance().m_fEffectsVolume = 0.9f;
	}
...
void CSoundManager::PlayAmbientSound3D(float fx, float fy, float fz, const char* c_szFileName, int iPlayCount)
{
	int iIndex = ms_SoundManager3D.SetInstance(c_szFileName);
	if (-1 == iIndex)
		return;
	...
	FadeInMusic(c_szFileName);

	pInstance->SetPosition((fx - m_fxPosition) / m_fEffectsScale,
		(fy - m_fyPosition) / m_fEffectsScale,
		(fz - m_fzPosition) / m_fEffectsScale);

	pInstance->SetVolume(GetEffectsVolume() / 20);
	pInstance->Play(iPlayCount);
}
...
void CSoundManager::SetSoundVolume3D(float fx, float fy, float fz, int iIndex, float fVolume)
{
	ISoundInstance * pInstance = ms_SoundManager3D.GetInstance(iIndex);
...
void CSoundManager::RestoreVolume()
{
	...
	SetEffectsVolume(m_fEffectsSoundVolume);
}
...
std::list<ActiveSound> activeSounds;

void CSoundManager::PlayMusic3D(DWORD dwIndex, float fx, float fy, float fz, const char* c_szFileName)
{
	float currentVolume = GetEffectsVolume();
	...
	pInstance->SetVolume(currentVolume);
	pInstance->Play(1);

	// Add the sound instance to the activeSounds list.
	activeSounds.push_back({ pInstance, dwIndex, currentVolume, true });
}

void CSoundManager::AdjustActiveSoundsVolume()
{
	float currentVolume = GetEffectsVolume()/3;
	float soundVolume;
	for (auto& sound : activeSounds)
	{
		if (sound.instance && sound.is3D)  // check if the sound instance exists and if it's a 3D sound
		{
			sound.instance->GetVolume(soundVolume);  // Get the current volume of the sound instance
			if (soundVolume != currentVolume)
			{
				sound.instance->SetVolume(currentVolume);
				sound.targetVolume = currentVolume;
			}
		}
	}
}
